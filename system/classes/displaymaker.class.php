...
class DisplayMaker
{
	...
	public function add_date($name, $timestamp)
	{
		...
		return $this->add_content($name, date($format, strtotime($timestamp)));
	}
	...
	public function make_display($cleanup = false)
	{
		...
		{
			$tag = '{# ' . $key . ' #}';
			$this->main_string = str_replace($tag, $value, $this->main_string);
			
		}		
		
		/*
		benchmark note- It might provide better performance if we changed the above to this commented code
			I don't want to assume so, however, so I'll leave it commented until its compared. ~tedivm
		...
		$this->main_string = preg_replace($tags, $content, $this->main_string);
		...
		*/
		...
		if($cleanup)
			$this->main_string = preg_replace('{\{# (.*?) #\}}', '', $this->main_string);	
